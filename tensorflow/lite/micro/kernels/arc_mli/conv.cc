...
TfLiteStatus EvalMliQuantizedPerChannel(
  ...
  if ((input->type == kTfLiteInt8) && (params->dilation_width_factor == 1) &&
    ...
    ConvertToMliTensor(input, &mli_in);
    ConvertToMliTensorPerChannel(filter, &mli_weights);
    ConvertToMliTensorPerChannel(bias, &mli_bias);
    ConvertToMliTensor(output, &mli_out);
